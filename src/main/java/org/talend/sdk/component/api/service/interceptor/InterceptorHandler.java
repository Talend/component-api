...
public interface InterceptorHandler {
    ...
    default <T extends Annotation> Optional<T> findAnnotation(final Method method, final Class<T> type) {
        return Optional.ofNullable((T) Stream.of(method.getAnnotations()).filter(a -> a.annotationType() == type).findFirst()
                .orElseGet(() -> Stream.of(method.getDeclaringClass().getAnnotations()).filter(a -> a.annotationType() == type)
